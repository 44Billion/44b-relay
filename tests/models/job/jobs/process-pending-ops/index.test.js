...
describe('Job: Process Pending Ops', () => {
  ...
  describe('run', () => {
    ...
    it('should process pruneCheck and trigger pruning if limit exceeded', async () => {
      ...
      const continued = await mdb.index('pendingOps').getDocument(op.key)
      assert.equal(continued.phase, 'queued')
      assert.equal(continued.data.targetBytes, 900)
      assert.equal(continued.data.step, 1)
    })
  })
})
